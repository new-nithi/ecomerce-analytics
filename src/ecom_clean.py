...
Original file is located at
    https://colab.research.google.com/drive/1XakpHyWsHXLt1Xh2eEXPb_LMKN9BoVJu
"""

# === CLEAN ECOM DATA (ONE-CELL) ===
import pandas as pd
import numpy as np
...
def load_df():
    try:
        return out.copy()  # จากขั้นตอน clean ที่ทำไว้
    except NameError:
        pass
...
# ยอดเงิน (เผื่อ line_total หาย/เพี้ยน)
...
x = df.copy()  # หรือ out.copy() ถ้าคุณใช้ out
...
# รายได้ต่อเดือน (เฉพาะ Completed)
m_rev = (
    x.loc[mask_cmp]
...
# ---------- PRODUCT TOP10 ----------
key = "product_name" if "product_name" in df.columns else ("product_id" if "product_id" in df.columns else None)
top_prod = top_table([key], n=10) if key else pd.DataFrame()
